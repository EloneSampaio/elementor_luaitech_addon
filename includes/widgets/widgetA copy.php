...
class WidgetA extends Widget_Base
{
	...
	protected function register_controls()
	{
		...
		$repeater->add_control(
			'image_formas',
			[
				'label' => esc_html__('Selecionar Imagem das formas', 'elementor-luaitech-addon'),
				'type' => \Elementor\Controls_Manager::MEDIA,
				'default' => [
					'url' => \Elementor\Utils::get_placeholder_image_src(),
				],
	...
	protected function render()
	{
		...
		if ($settings['slide_card']) {  ?>
			...
			<main id="teste">
								...
								<div class="swiper-container">
									...
									<div class="swiper-wrapper">
										...
										foreach ($settings['slide_card'] as $slide) {
											...
											$image_formas = \Elementor\Group_Control_Image_Size::get_attachment_image_src($slide['image_formas']['id'], 'thumbnail', $settings);

										?>
											<div class="swiper-slide" data-description="<?php esc_attr_e($slide['slide_description']); ?>" data-title="<?php esc_attr_e($slide['slide_titulo']); ?>" data-fundo="<?php esc_attr_e($fundo_url); ?>" data-forma="<?php esc_attr_e($image_formas) ?>">
				...
				<!--Show case end-->
				<div class="font-sans text-gray-900 bg-1" id="conteudo" style="	background: url('<?php echo plugins_url('assets/images/cor/01.png', ELEMENTOR_LUAITECH_ADDON_FILE);  ?>') no-repeat center center/cover;">
					...
					<div class="flex flex-wrap-reverse gap-y-24 justify-between py-7 px-6 mx-auto max-w-screen-xl sm:px-8 md:px-12 lg:px-16 xl:px-24">
						<div class="relative z-10 md:w-1/2 w-full">
							<img src="<?php echo plugins_url('assets/images/third.png', ELEMENTOR_LUAITECH_ADDON_FILE);  ?>" alt="" class="w-5/6 h-5/6 sm:w-4/4 sm:h-4/4 md:w-full md:h-full img-2" id="image_formas">
						</div>

						<div class="relative md:w-1/2 w-full flex flex-col justify-between">
							<img src="<?php echo plugins_url('assets/images/tree.png', ELEMENTOR_LUAITECH_ADDON_FILE);  ?>" alt="" class="w-4/6 h-50 ml-10 lg:self-end" id="tree">
						</div>
					</div>
				</div>
		...
		<script>
			...
			jQuery(document).ready(function() {
				jQuery('.swiper-slide img').click(function() {
					...
					var image_formas = jQuery(this).parent().attr('data-forma');
					console.log(image_formas + ' ok');
					...
					//change background da div de informações do produto
					jQuery('#conteudo').css('background-image', 'url(' + fundo + ')');

					//change backgroun d

					//change src image tag
					jQuery('#image_formas').attr('src', image_formas);
